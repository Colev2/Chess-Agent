# -*- coding: utf-8 -*-
"""chess_agent_nefeli_rl.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/11Dox8e_9SPUKTssrncGqK_kK23KSbE62
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
chess_agent_nefeli_rl.py

Hard-coded Nefeli RL version for the AUTH Nefeli cluster.

Goal:
  - Keep the original Colab chess-agent logic as unchanged as possible.
  - Remove Colab/Drive-specific code.
  - Assume supervised trainin([hpc.it.auth.gr](https://hpc.it.auth.gr/nefeli-partitions/)).
  - Run RL on Nefeli with vectorized batched self-play.

Recommended Nefeli allocation:
  partition: a100
  GPUs:      1
  CPUs:      32

Expected directory layout:
  $HOME/chess-agent-nefeli/
    outputs/
      checkpoints/
        latest_sl.pt                  optional fallback
      rl_checkpoints/
        model_latest.pt               preferred starting model copied from Colab/SL
        model_after_sl.pt             optional backup
        model_best.pt                 legacy/optional; not used by current RL loop
        rl_latest.pt                  full resume checkpoint, includes replay buffer
        rl_iter_XXXX.pt               archival checkpoints, no replay buffer

Run inside a Slurm job:
  python chess_agent_nefeli_rl.py train_rl

Important design choice:
  - RL runs sequentially on one GPU: first vectorized self-play, then NN training.
  - Self-play uses batched NN inference on the GPU, not per-worker CPU inference.
  - There is no arena gate, no promotion threshold, and no reset-to-best step.
"""
...
RL_CHECKPOINT_DIR = OUTPUT_DIR / "rl_checkpoints"
DATA_DIR = PROJECT_DIR / "data"
SL_SHARD_DIR = Path(
    os.environ.get(
        "SL_SHARD_DIR",
        str(OUTPUT_DIR / "sl_shards")
    )
)

for d in [CHECKPOINT_DIR, RL_CHECKPOINT_DIR, DATA_DIR, SL_SHARD_DIR]:
    d.mkdir(parents=True, exist_ok=True)

SL_LATEST_PATH = CHECKPOINT_DIR / "latest_sl.pt"
RL_LATEST_PATH = RL_CHECKPOINT_DIR / "rl_latest.pt"
MODEL_LATEST_PATH = RL_CHECKPOINT_DIR / "model_latest.pt"
MODEL_BEST_PATH = RL_CHECKPOINT_DIR / "model_best.pt"
MODEL_AFTER_SL_PATH = RL_CHECKPOINT_DIR / "model_after_sl.pt"
...
def train_rl(fresh_rl_from_sl=False):
    ...
    print("Vector batch games:", VECTOR_BATCH_GAMES, flush=True)
    ...
    for iteration in range(start_iteration, start_iteration + RL_ITERATIONS):
        ...
        if current_iter % 5 == 0:
            eval_path = RL_CHECKPOINT_DIR / f"model_iter_{current_iter}.pt"
